...
def generate_candidates(
    ...
    *,
    ...
    # 6) 얕은 리서치 — Top-K 전반 (병렬)
    shallow_reports: dict[str, ShallowReport] = {}

    def _run_shallow(e: Event):
        return e.id, shallow_fn(e)

    with ThreadPoolExecutor(max_workers=8) as pool:
        futs = {pool.submit(_run_shallow, e): e for e in events}
        for fut in as_completed(futs):
            e = futs[fut]
            try:
                eid, report = fut.result()
                shallow_reports[eid] = report
    ...
    def _run_deep(eid: str):
        e = events_by_id.get(eid)
        sh = shallow_reports.get(eid)
        if e is None or sh is None:
            return eid, None
        report, _evidence = deep_fn(e, sh)
        return eid, report.model_dump()

    with ThreadPoolExecutor(max_workers=3) as pool:
        futs = {pool.submit(_run_deep, eid): eid for eid in deep_targets}
        for fut in as_completed(futs):
            eid = futs[fut]
            try:
                _, result = fut.result()
                if result is not None:
                    deep_reports[eid] = result
            except Exception as ex:  # noqa: BLE001
    ...
    # 11) 내러티브 (병렬)
    if config.narrate:
        def _narrate(s: Story) -> Story:
            return narrative_fn(s, events_by_id, deep_reports)

        with ThreadPoolExecutor(max_workers=8) as pool:
            stories = list(pool.map(_narrate, stories))
