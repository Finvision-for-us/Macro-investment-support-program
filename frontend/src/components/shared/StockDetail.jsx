...
const METRIC_INFO = {
  ...
  asset_turnover:   { label: '자산회전율',       desc: '매출 ÷ 평균 총자산. 자산을 얼마나 효율적으로 활용해 매출을 창출하는지.', avgKey: 'asset_turnover', unit: '회', format: v => v?.toFixed(2) },
  inventory_turnover: { label: '재고회전율',     desc: '매출원가 ÷ 평균 재고. 재고가 얼마나 빠르게 판매되는지. 높을수록 효율적.', avgKey: 'inventory_turnover', unit: '회', format: v => v?.toFixed(1) },
  receivables_turnover: { label: '매출채권회전율', desc: '매출 ÷ 평균 매출채권. 외상 매출 수금 속도. 높을수록 현금화 빠름.', avgKey: 'receivables_turnover', unit: '회', format: v => v?.toFixed(1) },
  roic:             { label: 'ROIC',             desc: '세후영업이익(NOPAT) ÷ 투하자본(총자산−유동부채). 자본 효율성 핵심 지표.', avgKey: 'roic', unit: '%', format: v => v?.toFixed(1) + '%' },
